    ...
    return html`
      <div class="zs-datagrid-container" style="height: ${this.gridHeight}">
        <zentto-grid
          ...
          ?enable-configurator="${this.designMode || ((this.config?.props?.enableConfigurator as boolean) ?? false)}"
          ...
          ?enable-context-menu="${(this.config?.props?.enableContextMenu as boolean) ?? this.designMode}"
          ?enable-header-menu="${this.designMode}"
          ?enable-find="${(this.config?.props?.enableFind as boolean) ?? false}"
          ?enable-quick-search="${this.enableSearch}"
          ?enable-status-bar="${(this.config?.props?.enableStatusBar as boolean) ?? false}"
          ?enable-filter-panel="${(this.config?.props?.enableFilterPanel as boolean) ?? false}"
          ...
          grid-id="${(this.config?.props?.gridId as string) ?? ''}"
